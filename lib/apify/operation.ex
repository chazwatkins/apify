defmodule Apify.Operation do
  ...
  > are useful for plugins. See `Apify.Plugin` for more information.
  ...
  ## Examples
      ...
      iex> operation = %Operation{private: %{__opts__: [server: "https://api.apify.com"]}}
      iex> Operation.get_options(operation)
      [server: "https://api.apify.com"]
  ...
  @doc """
  Encode the request body using Jason

  This function encodes the request body using Jason.
  """
  @spec encode_body(t) :: t
  def encode_body(%__MODULE__{request_body: nil} = operation), do: operation

  def encode_body(%__MODULE__{request_body: body} = operation) do
    %__MODULE__{operation | request_body: body}
  end
  ...
  defp put_user_agent(operation) do
    user_agent =
      IO.iodata_to_binary([
        ...
        " via apify_client ",
        Application.spec(:apify, :vsn) || "0.1.0",
        "; Elixir ",
        System.version(),
        " / OTP ",
        System.otp_release()
